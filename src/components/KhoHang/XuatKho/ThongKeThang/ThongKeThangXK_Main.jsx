...
import ThongKeThangFilter from './ThongKeThangXK_Filter';
import { filterThongKeThang } from './ThongKeThangXK_FilterLogic';
import ThongKeThang_Export from './ThongKeThangXK_Export';
import '../../../utils/css/Highlight.css';
import '../../../utils/css/Custom-Button.css';
import '../../../utils/css/Custom-Filter.css';
import './ThongKeThangXK_Main.css';
...
const allMaHang = Array.from(new Set(data.map(item => item.ma_hang)));
  ...
  filteredRawData.forEach(item => {
    const month = new Date(item.ngay_xuat_hang).getMonth();
    const maHang = item.ma_hang;
    groupedData[maHang][month] += item.so_luong_xuat;
    groupedData[maHang].total += item.so_luong_xuat;
  });
  ...
  return (
    <div className="thong-ke-thang-container">
      <AreaHeader
        title="Tổng số lượng hàng xuất theo tháng"
