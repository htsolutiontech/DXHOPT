...
function TTBaoGia_Export({ data, filteredData, sortedData, onClose, visible }) {
  const [exporting, setExporting] = useState(false);
  const [activeTab, setActiveTab] = useState('1');

  const fieldMappings = {
    stt: 'STT',
    ma_trang_thai_bao_gia: 'Mã trạng thái báo giá',
    trang_thai_bao_gia: 'Trạng thái báo giá',
  ...
  const [exportOptions, setExportOptions] = useState({
    ...
    fileName: `trang_thai_bao_gia_${new Date().toISOString().split('T')[0]}`,
  ...
  return (
    <Modal
      ...
      title={<div className="export-modal-title"><FileExcelOutlined /> Xuất dữ liệu trạng thái báo giá</div>}
      open={visible}
      onCancel={onClose}
      width={700}
      footer={[
...
export default TTBaoGia_Export;