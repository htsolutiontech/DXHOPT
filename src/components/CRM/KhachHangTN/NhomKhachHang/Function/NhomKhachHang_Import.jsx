...
import {
  fetchPreviewData,
  renderMaNhomKhachHang,
  renderNhomKhachHang,
  renderNguoiCapNhat,
  isCustomerGroupExisting,
} from './NhomKhachHang_ImportRender';
import renderPreview from '../../../../utils/import/renderPreview';
import { createItem } from '../../../../utils/api/requestHelpers';

const { Dragger } = Upload;

const NhomKhachHang_Import = ({ open, onClose, onSuccess, disabled }) => {
  // State quản lý dữ liệu
  const [existingCustomerGroups, setExistingCustomerGroups] = useState([]);
  ...
  useEffect(() => {
    fetchPreviewData(setAccounts, setExistingCustomerGroups);
    resetState();
  }, [open]);

  // Hàm sinh mã nhóm khách hàng mới
  const generateMaNhomKhachHang = (() => {
    let base = null;
    return (index = 0) => {
      if (base === null) {
        let maxNumber = 0;
        existingCustomerGroups.forEach(item => {
          if (item.ma_nhom_khach_hang) {
      ...
      return `NKH${String(base + index + 1).padStart(2, '0')}`;
    };
  })();

  // Hàm xử lý sau khi parse file Excel
  const handleAfterParse = (parsedRows) => {
    // Gán mã nhóm khách hàng tự động cho từng dòng
    const dataWithMaLHD = parsedRows.map((row, idx) => ({
      ...row,
      ma_nhom_khach_hang: generateMaNhomKhachHang(idx)
    }));
  ...
  // Các trường bắt buộc
  const requiredFields = ['nhom_khach_hang', 'nguoi_cap_nhat'];
  const uniqueFields = ['nhom_khach_hang'];
  ...
  const handleValidateData = (data) => {
    ...
    return validateData(
      ...
      'nhom_khach_hang',
      {
        nhom_khach_hang: (value) => isCustomerGroupExisting(value, existingCustomerGroups),
      },
  ...
  const importSingleItem = async (item) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/customer-groups', item);
  ...
  const importAllItems = async (data) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/customer-groups', data);
      return result?.success;
    } catch (error) {
  ...
  const resetState = () => {
    ...
    generateMaNhomKhachHang.base = null;
  };
  ...
  const previewColumns = [
    ...
    { 
      ...
      width: "10%",
      render: (text, record) => renderMaNhomKhachHang(text, record, errorItems, existingCustomerGroups)
    },
    { 
      ...
      key: 'nhom_khach_hang', 
      width: "20%",
      render: (text, record) => renderNhomKhachHang(text, record, errorItems, existingCustomerGroups)
    },
    {
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleData = [
      ['Tập Đoàn', 'Hứa Tường Huy', ''],
      ['Bệnh viện Thầu', 'Hứa Tường Huy', '']
    ];
    downloadTemplate(columns, sampleData, 'Template_Nhom_Khach_Hang');
  };
            ...
            <Alert
              ...
              description={
                <ol>
                  ...
                  <li>Điền thông tin nhóm khách hàng vào file (mỗi dòng là một nhóm khách hàng).</li>
                  ...
                  <li>Các trường bắt buộc: Nhóm khách hàng, Người cập nhật.</li>
                </ol>
              }
              type="info"
              showIcon
...
export default NhomKhachHang_Import;