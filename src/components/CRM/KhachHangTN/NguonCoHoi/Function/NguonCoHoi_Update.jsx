...
const EditSource = ({ sourceId, onCancel, onSuccess }) => {
  ...
  useEffect(() => {
    if (sourceId) fetchContractTypeData(sourceId);
  ...
  const fetchContractTypeData = async (id) => {
    ...
    try {
      ...
      const opportunity_source = allSources.find(item => item.ma_nguon === id);
      if (!opportunity_source) throw new Error(`Không tìm thấy nguồn cơ hội với mã: ${id}`);
      if (opportunity_source.ngay_cap_nhat) opportunity_source.ngay_cap_nhat = moment(opportunity_source.ngay_cap_nhat);
      // Gán luôn người cập nhật là user hiện tại
      opportunity_source.nguoi_cap_nhat = currentUser?.ma_nguoi_dung || undefined;
      setSourceData(opportunity_source);
      form.setFieldsValue(opportunity_source);
      message.success(`Đã tải thông tin nguồn cơ hội: ${opportunity_source.nguon}`);
    } catch (error) {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const response = await updateItemById(`https://dx.hoangphucthanh.vn:3000/crm/opportunity-sources/${sourceId}`, payload);
        ...
        {fetchLoading ? (
          ...
          <>
            <h2 className="edit-title" style={{ marginBottom: 24 }}>
              Chỉnh sửa Nguồn Cơ Hội: {sourceData?.nguon || sourceId}
            </h2>
            <Form form={form} layout="vertical" onFinish={onFinish} className="edit-form">
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item name="ma_nguon" label="Mã nguồn" rules={[{ required: true }]}>
                    <Input disabled />
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item name="nguon" label="Nguồn" rules={[{ required: true }]}>
                    <Input />
                  </Form.Item>
                </Col>
              </Row>
