...
function LoaiTuongTac_Export({ data, filteredData, sortedData, onClose, visible }) {
  const [exporting, setExporting] = useState(false);
  const [activeTab, setActiveTab] = useState('1');

  const fieldMappings = {
    stt: 'STT',
    ma_loai_tuong_tac: 'Mã loại tương tác',
    loai_tuong_tac: 'Tên loại tương tác',
  ...
  const [exportOptions, setExportOptions] = useState({
    ...
    fileName: `loai_tuong_tac_${new Date().toISOString().split('T')[0]}`,
  ...
  return (
    <Modal
      ...
      title={<div className="export-modal-title"><FileExcelOutlined /> Xuất dữ liệu loại tương tác</div>}
      open={visible}
      onCancel={onClose}
      width={700}
      footer={[
...
export default LoaiTuongTac_Export;