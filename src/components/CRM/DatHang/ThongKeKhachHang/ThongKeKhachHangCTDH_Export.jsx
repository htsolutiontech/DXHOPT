...
function ThongKeKhachHangCTDH_Export({ data, filteredData, onClose, visible, allMaKhach, matchedCustomer }) {
  ...
  const exportOptions = {
    ...
    fileFormat: 'xlsx',
    exportFields,
    fileName: `thong_ke_hang_dat_theo_khach_hang_${new Date().toISOString().split('T')[0]}`,
    includeHeaderRow: true
  };

  return (
    <Modal
      title={<div className="export-modal-title"><FileExcelOutlined /> Xuất thống kê hàng đặt theo khách hàng</div>}
      open={visible}
      onCancel={onClose}
      width={700}
      footer={[
...
export default ThongKeKhachHangCTDH_Export;