...
import {
  fetchPreviewData,
  renderMaTrangThaiBaoGia,
  renderTrangThaiBaoGia,
  renderNguoiCapNhat,
  isQuotationStatusExisting,
} from './TTBaoGia_ImportRender';
import renderPreview from '../../../utils/import/renderPreview';
import { createItem } from '../../../utils/api/requestHelpers';

const { Dragger } = Upload;

const TTBaoGia_Import = ({ open, onClose, onSuccess, disabled }) => {
  // State quản lý dữ liệu
  const [existingQuotationStatuses, setExistingQuotationStatuses] = useState([]);
  ...
  useEffect(() => {
    fetchPreviewData(setAccounts, setExistingQuotationStatuses);
    resetState();
  }, [open]);

  // Hàm sinh mã trạng thái báo giá mới
  const generateMaTrangThaiBaoGia = (() => {
    let base = null;
    return (index = 0) => {
      if (base === null) {
        let maxNumber = 0;
        existingQuotationStatuses.forEach(item => {
          if (item.ma_trang_thai_bao_gia) {
            const match = item.ma_trang_thai_bao_gia.match(/^TTBG(\d+)$/);
            if (match) {
      ...
      return `TTBG${String(base + index + 1).padStart(2, '0')}`;
    };
  })();

  // Hàm xử lý sau khi parse file Excel
  const handleAfterParse = (parsedRows) => {
    // Gán mã báo giá tự động cho từng dòng
    const dataWithMaTTBG = parsedRows.map((row, idx) => ({
      ...row,
      ma_trang_thai_bao_gia: generateMaTrangThaiBaoGia(idx)
    }));
    setParsedData(dataWithMaTTBG);
    handleValidateData(dataWithMaTTBG);
    setShowPreview(true);
  };

  // Mapping giữa tiêu đề cột Excel và các trường API
  const columnMapping = {
    'Trạng thái báo giá': 'trang_thai_bao_gia',
    'Người cập nhật': 'nguoi_cap_nhat',
  };

  // Các trường bắt buộc
  const requiredFields = ['trang_thai_bao_gia', 'nguoi_cap_nhat'];
  const uniqueFields = ['trang_thai_bao_gia'];
  ...
  const handleValidateData = (data) => {
    ...
    return validateData(
      ...
      'trang_thai_bao_gia',
      {
        trang_thai_bao_gia: (value) => isQuotationStatusExisting(value, existingQuotationStatuses),
      },
  ...
  const importSingleItem = async (item) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/quotation-statuses', item);
  ...
  const importAllItems = async (data) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/quotation-statuses', data);
      return result?.success;
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (success) {
        message.success(`Đã nhập ${dataToImport.length} trạng thái báo giá thành công!`);
        fetchPreviewData(setAccounts, setExistingQuotationStatuses);
        resetState();
        onSuccess?.();
        onClose();
      ...
      message.info('Thử một cách khác - tạo từng báo giá một...');
      ...
      if (successCount > 0) {
        message.success(`Đã nhập ${successCount}/${parsedData.length} trạng thái báo giá thành công!`);
      } else {
        message.error('Không thể nhập được trạng thái báo giá nào!');
      }
      fetchPreviewData(setAccounts, setExistingQuotationStatuses);
      resetState();
      onSuccess?.();
  ...
  const resetState = () => {
    ...
    generateMaTrangThaiBaoGia.base = null;
  };
  ...
  const previewColumns = [
    ...
    { 
      title: 'Mã trạng thái báo giá', 
      dataIndex: 'ma_trang_thai_bao_gia', 
      key: 'ma_trang_thai_bao_gia', 
      width: "10%",
      render: (text, record) => renderMaTrangThaiBaoGia(text, record, errorItems, existingQuotationStatuses)
    },
    { 
      ...
      width: "20%",
      render: (text, record) => renderTrangThaiBaoGia(text, record, errorItems, existingQuotationStatuses)
    },
    {
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleData = [
      ['Đã gửi báo giá', 'Phan Sang Nhựt', ''],
      ['Đã báo giá thành công', 'Phan Sang Nhựt', '']
    ];
    downloadTemplate(columns, sampleData, 'Template_Trang_Thai_Bao_Gia');
  };
  ...
  const renderPreviewSection = () => {
    return renderPreview({
      label: "Tổng số trạng thái báo giá",
    ...
    className="import-modal"
      title={
        <div className="import-modal-title">
          <UploadOutlined /> Nhập danh sách trạng thái báo giá từ Excel
        </div>
      }
      open={open}
            ...
            <Alert
              ...
              description={
                <ol>
                  ...
                  <li>Điền thông tin báo giá vào file (mỗi dòng là một báo giá).</li>
                  ...
                  <li>Các trường bắt buộc: Trạng thái báo giá, Người cập nhật.</li>
                </ol>
              }
              type="info"
              showIcon
...
export default TTBaoGia_Import;