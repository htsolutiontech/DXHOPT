...
import {
  ...
  renderLoaiBaoGia,
  renderNguoiCapNhat,
  isQuotationTypeExisting,
} from './LoaiBaoGia_ImportRender';
import renderPreview from '../../../utils/import/renderPreview';
import { createItem } from '../../../utils/api/requestHelpers';

const { Dragger } = Upload;

const LoaiBaoGia_Import = ({ open, onClose, onSuccess, disabled }) => {
  // State quản lý dữ liệu
  const [existingQuotationTypes, setExistingQuotationTypes] = useState([]);
  ...
  useEffect(() => {
    fetchPreviewData(setAccounts, setExistingQuotationTypes);
    resetState();
  }, [open]);

  // Hàm sinh mã loại báo giá mới
  const generateMaLoaiBaoGia = (() => {
    let base = null;
    return (index = 0) => {
      if (base === null) {
        let maxNumber = 0;
        existingQuotationTypes.forEach(item => {
          if (item.ma_loai_bao_gia) {
            const match = item.ma_loai_bao_gia.match(/^LBG(\d+)$/);
            if (match) {
      ...
      return `LBG${String(base + index + 1).padStart(2, '0')}`;
    };
  })();
  ...
  const handleAfterParse = (parsedRows) => {
    ...
    const dataWithMaLBG = parsedRows.map((row, idx) => ({
      ...row,
      ma_loai_bao_gia: generateMaLoaiBaoGia(idx)
    }));
    setParsedData(dataWithMaLBG);
    handleValidateData(dataWithMaLBG);
    setShowPreview(true);
  };

  // Mapping giữa tiêu đề cột Excel và các trường API
  const columnMapping = {
    'Loại báo giá': 'loai_bao_gia',
    'Người cập nhật': 'nguoi_cap_nhat',
  };

  // Các trường bắt buộc
  const requiredFields = ['loai_bao_gia', 'nguoi_cap_nhat'];
  const uniqueFields = ['loai_bao_gia'];
  ...
  const handleValidateData = (data) => {
    ...
    return validateData(
      ...
      'loai_bao_gia',
      {
        loai_bao_gia: (value) => isQuotationTypeExisting(value, existingQuotationTypes),
      },
  ...
  const importSingleItem = async (item) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/quotation-types', item);
  ...
  const importAllItems = async (data) => {
    try {
      const result = await createItem('https://dx.hoangphucthanh.vn:3000/crm/quotation-types', data);
      return result?.success;
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (success) {
        message.success(`Đã nhập ${dataToImport.length} loại báo giá thành công!`);
        fetchPreviewData(setAccounts, setExistingQuotationTypes);
        resetState();
        onSuccess?.();
        onClose();
      ...
      if (successCount > 0) {
        message.success(`Đã nhập ${successCount}/${parsedData.length} loại báo giá thành công!`);
      } else {
        message.error('Không thể nhập được loại báo giá nào!');
      }
      fetchPreviewData(setAccounts, setExistingQuotationTypes);
      resetState();
      onSuccess?.();
  ...
  const resetState = () => {
    ...
    generateMaLoaiBaoGia.base = null;
  };
  ...
  const previewColumns = [
    ...
    { 
      ...
      dataIndex: 'ma_loai_bao_gia', 
      key: 'ma_loai_bao_gia', 
      width: "10%",
      render: (text, record) => renderMaLoaiBaoGia(text, record, errorItems, existingQuotationTypes)
    },
    { 
      title: 'Loại báo giá', 
      dataIndex: 'loai_bao_gia', 
      key: 'loai_bao_gia', 
      width: "20%",
      render: (text, record) => renderLoaiBaoGia(text, record, errorItems, existingQuotationTypes)
    },
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleData = [
      ['Báo giá nhóm mã hàng', 'Phan Sang Nhựt', ''],
      ['Báo giá Tập Đoàn Hoàn Mỹ', 'Phan Sang Nhựt', '']
    ];
    downloadTemplate(columns, sampleData, 'Template_Loai_Bao_Gia');
  };
    ...
    className="import-modal"
      title={
        <div className="import-modal-title">
          <UploadOutlined /> Nhập danh sách báo giá từ Excel
        </div>
      }
      open={open}
            ...
            <Alert
              ...
              description={
                <ol>
                  ...
                  <li>Các trường bắt buộc: Loại báo giá, Người cập nhật.</li>
                </ol>
              }
              type="info"
              showIcon
...
export default LoaiBaoGia_Import;