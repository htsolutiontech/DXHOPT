...
const AddQuotationType = ({ onCancel, onSuccess, disabled }) => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [fetchLoading, setFetchLoading] = useState(true);
  const [newMaLBG, setNewMaLBG] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchMaxSTT = async () => {
      ...
      try {
        const allQuotation_Types = await fetchDataList('https://dx.hoangphucthanh.vn:3000/crm/quotation-types');
        const maxSTT = allQuotation_Types.length ? Math.max(...allQuotation_Types.map(item => item.stt || 0)) : 0;
        const newSTT = maxSTT + 1;
        const generatedMaLBG = `LBG${String(newSTT).padStart(2, '0')}`;
        setNewMaLBG(generatedMaLBG);
  
        // Gán luôn giá trị mặc định vào form
        form.setFieldsValue({
          ma_loai_bao_gia: generatedMaLBG,
        ...
        message.error('Không thể khởi tạo mã loại báo giá mới');
      } finally {
    ...
    const onFinish = async (values) => {
      ...
      try {
        ...
        const response = await createItem('https://dx.hoangphucthanh.vn:3000/crm/quotation-types', payload);
        ...
        message.success('Thêm mới loại báo giá thành công!');
        onSuccess?.(); // Callback reload data
      } catch (error) {
        console.error('Lỗi thêm mới:', error);
        message.error('Không thể thêm mới loại báo giá');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="edit-container">
      {fetchLoading ? (
        ...
        <>
          <h2 className="edit-title" style={{ marginBottom: 24 }}>Thêm mới Loại Báo Giá</h2>
          <Form form={form} layout="vertical" onFinish={onFinish} className="edit-form">
            <Row gutter={16}>
              ...
              <Col span={12}>
                <Form.Item name="loai_bao_gia" label="Loại báo giá" rules={[{ required: true }]}>
                  <Input />
                </Form.Item>
              </Col>
            </Row>
...
export default AddQuotationType;
