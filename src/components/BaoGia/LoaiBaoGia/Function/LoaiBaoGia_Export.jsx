...
function LoaiBaoGia_Export({ data, filteredData, sortedData, onClose, visible }) {
  const [exporting, setExporting] = useState(false);
  const [activeTab, setActiveTab] = useState('1');

  const fieldMappings = {
    stt: 'STT',
    ma_loai_bao_gia: 'Mã loại báo giá',
  ...
  const [exportOptions, setExportOptions] = useState({
    ...
    fileName: `loai_bao_gia_${new Date().toISOString().split('T')[0]}`,
  ...
  return (
    <Modal
      ...
      title={<div className="export-modal-title"><FileExcelOutlined /> Xuất dữ liệu loại báo giá</div>}
      open={visible}
      onCancel={onClose}
      width={700}
      footer={[
...
export default LoaiBaoGia_Export;